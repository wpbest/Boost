    ...
    (stream_offset off, BOOST_IOS::seekdir way)
{
	...
	LARGE_INTEGER lDistanceToMoveHigh;
	lDistanceToMove.QuadPart = static_cast<LONGLONG>(off & 0xffffffff);
	lDistanceToMoveHigh.QuadPart = static_cast<LONGLONG>(off >> 32);
	DWORD dwResultLow =
		...
		::GetLastError() != NO_ERROR)
	{
		...
		return offset_to_position(
			(stream_offset(lDistanceToMoveHigh.QuadPart) << 32) + dwResultLow
			);
	}
#else // BOOST_PLAT_WINDOWS_RUNTIME
