...
void mapped_file_impl::resize(stream_offset new_size)
{
	...
	LARGE_INTEGER tmp2;
	tmp1.QuadPart = 0;
	stream_offset offset = ::SetFilePointerEx(handle_, tmp1, NULL, FILE_CURRENT);
	if (offset == INVALID_SET_FILE_POINTER && ::GetLastError() != NO_ERROR)
		cleanup_and_throw("failed querying file pointer");
	LONGLONG sizehigh = (new_size >> (sizeof(LONG) * 8));
	LONGLONG sizelow = (new_size & 0xffffffff);
	tmp1.QuadPart = sizelow;
	tmp2.QuadPart = sizehigh;
	DWORD result = ::SetFilePointerEx(handle_, tmp1, &tmp2, FILE_BEGIN);
	if ((result == INVALID_SET_FILE_POINTER && ::GetLastError() != NO_ERROR)
		|| !::SetEndOfFile(handle_))
		cleanup_and_throw("failed resizing mapped file");
	tmp2.QuadPart = (offset >> (sizeof(LONG) * 8));
	tmp1.QuadPart = (offset & 0xffffffff);
	::SetFilePointerEx(handle_, tmp1, &tmp2, FILE_BEGIN);
#else // BOOST_PLAT_WINDOWS_RUNTIME
...
void mapped_file_impl::open_file(param_type p)
{
	...
	DWORD dwCreationDisposition = (p.new_file_size != 0 && !readonly) ? CREATE_ALWAYS : OPEN_EXISTING;
	...
	if (p.new_file_size != 0 && !readonly) {
		LONGLONG sizehigh = (p.new_file_size >> (sizeof(LONG) * 8));
		LONGLONG sizelow = (p.new_file_size & 0xffffffff);
		LARGE_INTEGER tmp1;
		LARGE_INTEGER tmp2;
		tmp1.QuadPart = sizelow;
		tmp2.QuadPart = sizehigh;
	...
	{
		boost::intmax_t size =
			((static_cast<boost::intmax_t>(info.EndOfFile.HighPart) << 32) |
...
void mapped_file_impl::try_map_file(param_type p)
{
...
#ifdef BOOST_PLAT_WINDOWS_RUNTIME
	// Create mapping
	ULONG protect = priv ? PAGE_WRITECOPY : readonly ? PAGE_READONLY : PAGE_READWRITE;
	mapped_handle_ = ::CreateFileMappingFromApp(
		handle_,
		NULL,
		protect,
		0,
	...
	// Access data
	ULONG access = priv ? FILE_MAP_COPY : readonly ? FILE_MAP_READ : FILE_MAP_WRITE;
